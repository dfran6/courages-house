...
export function Scene({ onProgressChange, backgroundColor,fogColor, fogStrength }) {
  ...
  return (
    <>
      ...
      {/*
        `enabled` is owned by ExperienceController, which flips it on at
        offset >= 0.8. Driving it from a prop here as well meant every React
        re-render fought the frame loop, using a different threshold (0.94).
      */}
      <OrbitControls
        ref={controlsRef}
        enabled={false}
