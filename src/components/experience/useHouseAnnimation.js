...
// const CAMERA_TARGET = [-10, 20, 0]
const CAMERA_TARGET = [0, 3, 0]


export function useHouseAnimation(houseRef, nodes) {
    ...
    useFrame((state, delta) => {
        ...
        const roofProgress = smoothstep(
            rangeProgress(scrollProgress, 0.45, 0.7)
        );
        ...
        // Main build rises into position
        ...
        const wallProgress = smoothstep(
            rangeProgress(scrollProgress, 0.15, 0.45)
        );
        ...
        // Left wall enters from the left
        ...
        // Right wall enters from the right
        ...
        // Entrance wall comes from the front
        ...
        const detailProgress = smoothstep(
            rangeProgress(scrollProgress, 0.7, 0.85)
        );

        // briks rise into place
        nodes.briks.position.y =
            original.briks.position.y +
            lerp(-10, 0, detailProgress);

        // Support beams slide into place
        nodes.support_woods.position.y =
            original.support_woods.position.y + lerp(-10, 0, detailProgress);

        // windows slide into place
        animatePart(nodes.window, original.window, detailProgress, 80)
        animatePart(nodes.window001, original.window001, detailProgress, 80);
        animatePart(nodes.window002, original.window002, detailProgress, 80);
        animatePart(nodes.window003, original.window003, detailProgress, 80);
        animatePart(nodes.window004, original.window004, detailProgress, 80);
        animatePart(nodes.window005, original.window005, detailProgress, 80);
        animatePart(nodes.window006, original.window006, detailProgress, 80);
        animatePart(nodes.window007, original.window007, detailProgress, 80);
        animatePart(nodes.window008, original.window008, detailProgress, 80);
        animatePart(nodes.window009, original.window009, detailProgress, 80);
        animatePart(nodes.window010, original.window010, detailProgress, 80);
        animatePart(nodes.window011, original.window011, detailProgress, 80);
        animatePart(nodes.door, original.door, detailProgress, 80);
        animatePart(nodes.backDoor, original.backDoor, detailProgress, 80);

        const camera = state.camera;
        const finalProgress = smoothstep(
            rangeProgress(scrollProgress, 0.85, 0.9)
        );

        animatePart(nodes.ironShimmy, original.ironShimmy, finalProgress, 80);
        animatePart(nodes.fanMotor, original.fanMotor, finalProgress, 80);
        animatePart(nodes.fanStand, original.fanStand, finalProgress, 80);
        animatePart(nodes.fanblades, original.fanblades, finalProgress, 80);
        ...
        if (finalProgress < 0.8) {
            camera.position.x = lerp(60, 45, finalProgress);
            camera.position.y = lerp(30, 10, finalProgress);
            camera.position.z = lerp(8, 2, finalProgress);
        ...
        if (finalProgress > 0.9) {
            // nodes.fanblades.rotation.x += 0.03;
